...
HookClient.prototype.enable_eventmode = function(callback) {
...
HookClient.prototype.on_data = function(data) {
	...
	else if (this.eventmode && /^(chat from|kill victim|login char|launch char|base(enter|exit) char|jumpin char|switchout char|spawn char|connect id|disconnect char)=/.test(data_array[0])) {
		this.response_queue.length = 0;
